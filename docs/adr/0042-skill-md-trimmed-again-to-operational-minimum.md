...
141 lines became 99: the sections above, plus one last redundancy caught on a second read, the opening paragraph restating "never compute, never infer, never send" in prose immediately above a Hard Rules section that already states the same three constraints as a checklist. Tightened to a one-line identity statement and let Hard Rules own the actual rules. Every command line is unchanged, byte for byte, confirmed by diff each time: `git diff` on this file shows zero differing lines matching `python `. Re-verified with the same clean-room dry run [ADR 0028](0028-verified-as-an-agent-skill.md) and the ADR 0036 trim both used, from a fresh scratch directory outside the repository: the exact same result, 50/46/4, corrections applied, 50/50, 5 mandatory and 2 no-letter, 48 letters, 0 rejections. Nothing about what the skill does changed; only how much a trigger-time read costs to find out.
